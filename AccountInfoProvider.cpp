...
namespace ingonline
{
	...
	{
		emit statusInfo("Sending balance request");
	...
	void AccountInfoProvider::onHttpReply(QNetworkReply *reply)
	{
		...
		if (state == LOGIN_ENTRED)
		{
			...
			{
				emit unexpectedError("SHA1 file not found");
			}
		...
		else if (state == PASSWORD_ENTRED)
		{
			...
			if (!reply->rawHeader("Location").contains("err"))
			{
				...
				emit statusInfo("Login successfull");
			}
			else
			{
				emit unexpectedError("Login unsuccessfull");
			}
		}
		...
		{
			emit statusInfo("Account history downloaded");
			...
			emit historyDataUpdated(historyEntries);
			emit statusInfo(QString("Last update: ") + QDateTime::currentDateTime().toString("hh:mm:ss dd/MM/yy"));
